...
class Home(QMainWindow, Ui_MainWindow):
    ...
    def generate_books(self, search, sort):
        """Creates objects for the books
        """
        self.books = []

        query = {
            'Title (A to Z)': 'name COLLATE NOCASE ASC',
            'Title (Z to A)': 'name COLLATE NOCASE DESC',
            'Rating (0 to 10)': 'rating ASC',
            'Rating (10 to 0)': 'rating DESC',
            'Pages (low to high)': 'pages ASC',
            'Pages (high to low)': 'pages DESC',
            'Date Added (old to new)': 'date_added ASC',
            'Date Added (new to old)': 'date_added DESC',
            'Randomly': 'random()'
        }

        for book in self.db.get_books(search, query[sort]):
            # set up frame
    ...
    def populate_gallery(self):
        ...
        self.generate_books(search, self.sort_by.currentText())
